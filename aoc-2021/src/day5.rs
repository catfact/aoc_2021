...
fn get_range (segments: &Vec<[[i32; 2]; 2]>) -> ((i32, i32), (i32,i32)) { 
    let xmin = segments.iter().map(|seg| seg[0][0].min(seg[1][0])).min().unwrap();
    let xmax = segments.iter().map(|seg| seg[0][0].max(seg[1][0])).max().unwrap();
    let ymin = segments.iter().map(|seg| seg[1][0].min(seg[1][1])).min().unwrap();
    let ymax = segments.iter().map(|seg| seg[1][0].max(seg[1][1])).max().unwrap();
    return ((xmin, ymin), (xmax, ymax));
}

pub fn test1() -> usize {
    let mut segments = parse_input();
    segments = segments
        .iter()
    ...
    for i in xmin..(xmax+1) { 
        for j in ymin..(ymax+1) {
            ...
            print!("{}", if z>&0 {z.to_string()} else {".".to_string()});
        }
        println!("");
    }
